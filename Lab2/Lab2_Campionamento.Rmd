...
## LAB2. Il principio del campionamento ripetuto

#### DISTRIBUZIONE NORMALE

```{r}
#FUNZIONE DI RIPARTIZIONE DELLA NORMALE
...
Generiamo un campione casuale da una normale standard
...
qqplot(qexp(ppoints(z), 2), sort(z)) # qexp mi restituisce i quantili della exp
abline(0, 1)
```

## Simulazioni di Montecarlo

## Distribuzione campionaria della media di un campione normale

vogliamo verificare la teoria, ovvero se è vera la distribuzione della media campionaria e varianza campionaria. TEROREMA DEL LIMITE CENTRALE
...
var.camp=sum((campione-media.camp)^2)/(n-1) # CORRETTA  s2
var.camp
var.camp.nc=sum((campione-media.camp)^2)/n # NON CORRETTA sigma2
var.camp.nc
```

proviamo a fare una simulazione di 1000 campioni

```{r}
#MC inizializzazione
N=1000 ## fissiamo il numero di campioni
medie = vector("numeric", length = N)  ##vettore delle medie vuoto
```

```{r}
#MC replicazioni
for (i in 1:N) {
  campione = rnorm(n, mean = 1, sd = 0.7) # n=20
  medie[i] = mean(campione) # mi salvo le media, che so che avranno una distrib NORMALE
}
```

```{r}
# vediamo se hanno una distribuzione normale
par(mfrow = c(1, 2))
#istogramma-densità
...
varianze = vector("numeric", length = N) # vettore delle varianze vuoto
for (i in 1:N) {
  campione = rnorm(n, mean = 1, sd = 0.7)
...
Vediamo se il Teorema del limite centrale vale anche per distribuzioni NON normali. Proviamo con la Gamma.
...
vogliamo vedere se sono vere le distribuzioni dei coefficienti di un modello di regressione lineare

beta1=5, beta2=3, sigma2=16

errorei sono distribuiti come una normale con media zero e sigma2=16

```{r}
# creiamo del rumore su dei dati distribuiti come y= 5 + 3x
n = 30
x = 1:n # sequenza da 1 a 30
error = rnorm(n, mean = 0, sd = 4)
y = 5 + 3 * x + error # aggiungo l'errore
plot(x, y)
```

```{r}
## Otteniamo le stime di beta_r, applico le formule
...
vediamo se è vero che sono distribuiti come una normale
...
for (i in 1:N) {
  ...
  rifiuto[i] = (abs(t2) > qt(1 - livalfa/2, n - 2)) # qt sono i quantili della Tstudent
}
mean(rifiuto)   #tasso di rifiuto
```

è molto vicino a 0.05.
...
N = 10000
beta2.ic = matrix(NA, ncol = 2, nrow = N) # buona norma inizializzare con NA
beta2.hat.sim = rep(NA, N)
beta1.hat.sim = rep(NA, N)
coperto = vector("numeric", length = N)
n = 30
